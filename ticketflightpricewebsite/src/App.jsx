...
function FlightPricePredictor() {
  ...
  const handleChange = (e) => {
    setFormData({
      ...formData,   //uses the spread operator to create a copy of the current formData object. This is important because you want to retain the existing values in formData.
      [e.target.name]: e.target.value,  //This means that the key (e.target.name) is dynamically set based on the input’s name attribute, and its value is set to e.target.value.

      //e.target.name allows you to dynamically access each form field based on its name attribute, 

      //For example, if an input has name="email" and a user types "user@example.com", this would update formData with { email: "user@example.com" }, keeping other fields intact.
    });
  };
